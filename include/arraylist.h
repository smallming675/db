...
static inline void alist_init(ArrayList *arr, size_t element_size,
                              ArrayListFree free_func) {
...
static inline void alist_init_with_cap(ArrayList *arr, size_t element_size,
                                       int capacity, ArrayListFree free_func) {
...
static inline void alist_grow(ArrayList *arr) {
  int new_capacity =
      arr->capacity == 0 ? ALIST_INIT_CAPACITY : arr->capacity * 2;
...
static inline void alist_set(ArrayList *arr, int index, const void *value) {
  if (index < 0 || index >= arr->length)
    return;
  memcpy((char *)arr->data + (index * arr->element_size), value,
         arr->element_size);
}
...
static inline int alist_length(const ArrayList *arr) { return arr->length; }

static inline int alist_capacity(const ArrayList *arr) { return arr->capacity; }

static inline void *alist_data(ArrayList *arr) { return arr->data; }
