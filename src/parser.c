...
static void print_error_line(FILE *stream, const char *fmt, ...);
static bool parse_date_literal(const char *value, int *year, int *month, int *day);
static bool parse_time_literal(const char *value, int *hour, int *minute, int *second);
static bool parse_hex_byte(const char *value, unsigned char *out);
...
static void free_expr(Expr *expr) {
    ...
    case EXPR_AGGREGATE_FUNC:
        if (expr->aggregate.operand)
            free_expr(expr->aggregate.operand);
        break;
    case EXPR_SCALAR_FUNC:
        for (int i = 0; i < expr->scalar.arg_count; i++) {
            if (expr->scalar.args[i])
                free_expr(expr->scalar.args[i]);
        }
        break;
    case EXPR_SUBQUERY:
...
static const char *get_token_type_desc(TokenType type) {
    ...
    default:
        log_msg(LOG_ERROR, "get_token_type_desc: Token type not found!");
        return "a token";
    }
}
...
static void parse_error_init(ParseContext *ctx, const char *input, Token *tokens, int token_count) {
    ctx->tokens = tokens;
...
static void parse_error_set(ParseContext *ctx, ParseErrorCode code, const char *message,
...
static const char *parse_error_code_str(ParseErrorCode code);

static void print_error_header(ParseContext *ctx) {
    print_error_line(stderr, COLOR_RESET COLOR_RED "%s" COLOR_RESET ": ",
                     parse_error_code_str(ctx->error.code));
    ctx->error.line = calculate_line_number(ctx->input, 0);
    print_error_line(stderr, "%s\n", ctx->error.message);
    print_error_line(stderr, "Input:" COLOR_RESET " " COLOR_YELLOW "%s" COLOR_RESET "\n",
                     ctx->input);
    print_error_line(stderr, "Expected:" COLOR_RESET " " COLOR_GREEN "%s" COLOR_RESET "\n",
                     ctx->error.expected);
    print_error_line(stderr, "Found:" COLOR_RESET " " COLOR_RED "%s" COLOR_RESET "\n",
                     ctx->error.found);
}

static void print_token_context(ParseContext *ctx) {
    print_error_line(stderr, "Error location (token %d):\n", ctx->error.token_index);
...
static int find_token_position(ParseContext *ctx) {
...
static void print_fix_suggestion(ParseContext *ctx) {
    fputs(COLOR_GREEN COLOR_BOLD "Fix:" COLOR_RESET "\n", stderr);
    ...
    while (sug_start < len) {
        ...
        for (size_t i = sug_start; i < end && i < sug_start + 70; i++) {
            fputc(ctx->error.suggestion[i], stderr);
        }
        fputs("\n", stderr);
...
void parse_error_report(ParseContext *ctx) {
    ...
    if (ctx->error.token_index >= 0 && ctx->error.token_index < ctx->token_count) {
        print_token_context(ctx);
        int token_pos = find_token_position(ctx);
        print_input_context_with_marker(ctx, token_pos);
    }

    print_fix_suggestion(ctx);
}
...
static void expect_get_eof_error_msg(const char *context, char *msg, size_t msg_size,
                                     char *suggestion, size_t sugg_size) {
...
static bool expect(ParseContext *ctx, TokenType type, const char *context) {
    ...
    if (current_token->type == TOKEN_EOF) {
        ...
        expect_get_eof_error_msg(context, msg, sizeof(msg), suggestion, sizeof(suggestion));
    } else {
...
static DataType parse_data_type(ParseContext *ctx) {
    if (consume(ctx, TOKEN_KEYWORD)) {
        ...
        if (strcasecmp(current_token[-1].value, "INTEGER") == 0)
            return TYPE_INT;
        if (strcasecmp(current_token[-1].value, "STRING") == 0)
            return TYPE_STRING;
        ...
        if (strcasecmp(current_token[-1].value, "DOUBLE") == 0)
            return TYPE_FLOAT;
        if (strcasecmp(current_token[-1].value, "BOOLEAN") == 0)
            return TYPE_BOOLEAN;
        if (strcasecmp(current_token[-1].value, "BOOL") == 0)
            return TYPE_BOOLEAN;
        if (strcasecmp(current_token[-1].value, "DECIMAL") == 0)
            return TYPE_DECIMAL;
        if (strcasecmp(current_token[-1].value, "NUMERIC") == 0)
            return TYPE_DECIMAL;
        if (strcasecmp(current_token[-1].value, "BLOB") == 0)
            return TYPE_BLOB;
    ...
    log_msg(LOG_ERROR, "parse_data_type: Invalid data type: %s", current_token[-1].value);
    return TYPE_INT;
}

static bool parse_column_name_and_type(ParseContext *ctx, ColumnDef *col) {
    if (!match(TOKEN_IDENTIFIER)) {
        char expected[256];
        string_format(expected, sizeof(expected), "column name (IDENTIFIER)");
...
static bool parse_column_def(ParseContext *ctx, ColumnDef *col) {
    if (!parse_column_name_and_type(ctx, col)) {
        return false;
    }
    ...
    while (current_token->type != TOKEN_COMMA && current_token->type != TOKEN_RPAREN &&
        ...
        if (current_token->type == TOKEN_NOT && current_token[1].type == TOKEN_NULL) {
            ...
            if (!parse_column_foreign_key_refs(ctx, col)) {
                return false;
            }
        } else if (current_token->type == TOKEN_REFERENCES) {
            if (!parse_column_references(ctx, col)) {
                return false;
            }
        } else if (current_token->type == TOKEN_KEYWORD) {
...
static Value parse_value(ParseContext *ctx) {
    ...
    if (match(TOKEN_STRING)) {
        ...
        val.type = TYPE_DATE;
        int year, month, day;
        if (parse_date_literal(current_token->value, &year, &month, &day)) {
        ...
        if (parse_time_literal(current_token->value, &hour, &minute, &second)) {
        ...
        if (val.blob_val.data) {
            for (size_t i = 0; i < val.blob_val.length; i++) {
                ...
                if (parse_hex_byte(&hex_str[i * 2], &byte)) {
                    ((unsigned char *)val.blob_val.data)[i] = byte;
                } else {
                    ((unsigned char *)val.blob_val.data)[i] = 0;
                }
            }
        }
        advance();
...
static void parse_literal_value(Expr *expr) {
...
static Expr *parse_parenthesized_expr(ParseContext *ctx) {
...
static Expr *parse_comparison_expr(ParseContext *ctx) {
    ...
    while (match(TOKEN_EQUALS) || match(TOKEN_NOT_EQUALS) || match(TOKEN_LESS) ||
        ...
        Expr *expr = malloc(sizeof(Expr));
        if (!expr) {
            log_msg(LOG_ERROR, "parse_comparison_expr: malloc() failed");
            free_expr(left);
            return NULL;
        }
        expr->alias[0] = '\0';
...
static Expr *parse_and_expr(ParseContext *ctx) {
    ...
    while (match(TOKEN_AND)) {
        ...
        Expr *expr = malloc(sizeof(Expr));
        if (!expr) {
            log_msg(LOG_ERROR, "parse_and_expr: malloc() failed");
            free_expr(left);
            return NULL;
        }
        expr->alias[0] = '\0';
...
static Table *parse_insert_table_and_columns(ParseContext *ctx, ASTNode *node,
                                             ArrayList *column_indices, bool *has_columns) {
...
static bool parse_insert_setup_columns(ASTNode *node, ArrayList *column_indices) {
    alist_init(&node->insert.columns, sizeof(int), NULL);
    if (!node->insert.columns.data) {
        return false;
    }
...
static bool parse_insert_value_rows(ParseContext *ctx, ASTNode *node, bool has_columns,
                                    int col_count, Table *table) {
...
static ASTNode *parse_insert(ParseContext *ctx) {
    ...
    Table *table = parse_insert_table_and_columns(ctx, node, &column_indices, &has_columns);
    if (!table)
        goto error;

    if (!parse_insert_setup_columns(node, &column_indices))
        goto error;

    int col_count = has_columns ? alist_length(&column_indices)
                                : (table ? alist_length(&table->schema.columns) : 0);
...
static bool parse_select_columns(ParseContext *ctx, ASTNode *node) {
    ...
    if (match(TOKEN_OPERATOR) && strcmp(current_token->value, "*") == 0) {
        ...
        Expr **expr = (Expr **)alist_append(&node->select.expressions);
        if (!expr) {
            if (star_expr) {
                free_expr(star_expr);
            }
            log_msg(LOG_ERROR, "Failed to allocate memory for expression");
            return false;
        }
        *expr = star_expr;
        advance();
...
static bool parse_select_from_table(ParseContext *ctx, ASTNode *node) {
...
static bool parse_select_join_clause(ParseContext *ctx, ASTNode *node) {
    if (match(TOKEN_JOIN)) {
...
static bool parse_select_order_by(ParseContext *ctx, ASTNode *node) {
...
static bool parse_select_limit(ParseContext *ctx, ASTNode *node) {
...
static ASTNode *parse_select(ParseContext *ctx) {
    ...
    if (!parse_select_from_table(ctx, node))
        goto error;
    if (!parse_select_join_clause(ctx, node))
        goto error;
    ...
    if (!parse_select_order_by(ctx, node))
        goto error;
    if (!parse_select_limit(ctx, node))
        goto error;
...
static Table *parse_update_table_and_set(ParseContext *ctx, ASTNode *node) {
...
static bool parse_update_assignments(ParseContext *ctx, ASTNode *node, Table *table) {
...
static ASTNode *parse_update(ParseContext *ctx) {
    ...
    Table *table = parse_update_table_and_set(ctx, node);
    if (!table)
        goto error;

    if (!parse_update_assignments(ctx, node, table))
        goto error;
...
static ASTNode *parse_index_name_and_allocate(ParseContext *ctx) {
    if (!match(TOKEN_IDENTIFIER)) {
...
static Table *parse_table_for_index(ParseContext *ctx, ASTNode *node) {
    if (!match(TOKEN_KEYWORD) || strcasecmp(current_token->value, "ON") != 0) {
...
static bool parse_column_for_index(ParseContext *ctx, ASTNode *node, Table *table) {
...
static ASTNode *parse_create_index(ParseContext *ctx) {
    ...
    ASTNode *node = parse_index_name_and_allocate(ctx);
    if (!node)
        return NULL;

    Table *table = parse_table_for_index(ctx, node);
    if (!table)
        goto error;

    if (!parse_column_for_index(ctx, node, table))
        goto error;
...
ASTNode *parse_with_context(ParseContext *ctx, Token *tokens) {
    ...
    parse_error_set(ctx, PARSE_ERROR_INVALID_SYNTAX, "Expected a SQL statement",
...
ASTNode *parse_ex(const char *input, Token *tokens) {
    ...
    int count = 0;
    if (tokens) {
        while (count < 10000 && count_tokens[count].type != TOKEN_EOF) {
            count++;
        }
        count++;
    }
...
void free_ast(ASTNode *ast) {
    ...
    case AST_UPDATE_ROW:
        ...
        if (ast->update.where_clause) {
            free_expr(ast->update.where_clause);
        }
        break;
    case AST_DELETE_ROW:
    ...
    case AST_SELECT: {
        ...
        if (ast->select.where_clause) {
            free_expr(ast->select.where_clause);
        }
        if (ast->select.join_condition) {
            free_expr(ast->select.join_condition);
        }
        break;
    }
    default:
...
static void print_error_line(FILE *stream, const char *fmt, ...) {
    ...
    va_list args;
    va_start(args, fmt);
    string_format_v(buffer, sizeof(buffer), fmt, args);
    va_end(args);
    fputs(buffer, stream);
}
