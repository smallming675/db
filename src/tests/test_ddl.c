...
void test_foreign_key_validation(void) {
    ...
    assert_int_eq(1, alist_length(&find_table_by_name("products")->rows),
                  "Should insert product with valid FK");

    exec("INSERT INTO products VALUES (102, 'Mouse', 1);");
    assert_int_eq(2, alist_length(&find_table_by_name("products")->rows),
                  "Should insert another product with valid FK");

    log_msg(LOG_INFO, "Testing FOREIGN KEY violation on INSERT...");

    exec("INSERT INTO products VALUES (103, 'Invalid', 999);");
    assert_int_eq(2, alist_length(&find_table_by_name("products")->rows),
                  "Should not insert product with invalid FK");

    exec("INSERT INTO products VALUES (103, 'Also Invalid', 3);");
    assert_int_eq(2, alist_length(&find_table_by_name("products")->rows),
                  "Should not insert product with non-existent FK");
    ...
    assert_int_eq(1, alist_length(&find_table_by_name("nullable_fk")->rows),
...
void test_foreign_key_self_reference(void) {
    ...
    exec("CREATE TABLE employees (emp_id INT PRIMARY KEY, name STRING, manager_id INT REFERENCES "
         "employees(emp_id));");
    ...
    assert_int_eq(3, alist_length(&find_table_by_name("employees")->rows),
                  "Should insert employees with valid self-reference");

    log_msg(LOG_INFO, "Testing FOREIGN KEY self-reference violation...");

    exec("INSERT INTO employees VALUES (4, 'Invalid', 999);");
    assert_int_eq(3, alist_length(&find_table_by_name("employees")->rows),
                  "Should not insert with invalid self-reference");
...
void test_foreign_key_multiple_references(void) {
    ...
    exec("CREATE TABLE orders (order_id INT PRIMARY KEY, customer_id INT REFERENCES "
         "customers(customer_id), supplier_id INT REFERENCES suppliers(supplier_id));");

    exec("INSERT INTO orders VALUES (1, 1, 1);");
    assert_int_eq(1, alist_length(&find_table_by_name("orders")->rows),
                  "Should insert order with valid FKs");

    exec("INSERT INTO orders VALUES (2, 2, 2);");
    assert_int_eq(2, alist_length(&find_table_by_name("orders")->rows),
                  "Should insert another order");

    log_msg(LOG_INFO, "Testing FOREIGN KEY multiple reference violations...");

    exec("INSERT INTO orders VALUES (3, 1, 999);");
    assert_int_eq(2, alist_length(&find_table_by_name("orders")->rows),
                  "Should not insert with invalid supplier FK");

    exec("INSERT INTO orders VALUES (3, 999, 1);");
    assert_int_eq(2, alist_length(&find_table_by_name("orders")->rows),
                  "Should not insert with invalid customer FK");
...
void test_create_table_multiple_columns(void) {
    ...
    exec("CREATE TABLE complex_table ("
         "id INT PRIMARY KEY, "
...
void test_create_table_new_types(void) {
    ...
    if (!col) {
        assert_ptr_not_null(col, "First column should exist");
        return;
    }
    assert_true(strcmp(col->name, "id") == 0, "First column should be 'id'");
    ...
    assert_true(strcmp(col->name, "active") == 0, "Second column should be 'active'");
    assert_int_eq(TYPE_BOOLEAN, col->type, "active should be BOOLEAN type");

    col = (ColumnDef *)alist_get(&table->schema.columns, 2);
    if (!col) {
        assert_ptr_not_null(col, "Third column should exist");
        return;
    }
    assert_true(strcmp(col->name, "price") == 0, "Third column should be 'price'");
    assert_int_eq(TYPE_DECIMAL, col->type, "price should be DECIMAL type");

    col = (ColumnDef *)alist_get(&table->schema.columns, 3);
    if (!col) {
        assert_ptr_not_null(col, "Fourth column should exist");
        return;
    }
    assert_true(strcmp(col->name, "balance") == 0, "Fourth column should be 'balance'");
    assert_int_eq(TYPE_DECIMAL, col->type, "balance should be NUMERIC type (DECIMAL)");

    col = (ColumnDef *)alist_get(&table->schema.columns, 4);
    if (!col) {
        assert_ptr_not_null(col, "Fifth column should exist");
        return;
    }
